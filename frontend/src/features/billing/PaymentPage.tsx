...
import { Banknote, Smartphone, CreditCard, Gift, Printer, CheckCircle2, Delete, Star, Ticket, BedDouble } from 'lucide-react';
...
/** Payment screen: large method buttons, keypad-style amount, split payments, no overpayment (server-enforced too). */
    ...
    <div>
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_400px]">
        <div className="space-y-4">
          {balance <= 0 ? (
            <Card className="text-center py-10"><CheckCircle2 className="h-12 w-12 text-success-600 mx-auto" /><h2 className="text-heading mt-3">Fully paid</h2><p className="text-neutral-500 mt-1">Print the receipt and close the order.</p><div className="mt-5 flex justify-center gap-2"><Button variant="outline" leftIcon={<Printer className="h-4 w-4" />} onClick={() => navigate(`/cashier/bills/${b.id}/receipt`)}>Receipt</Button><Button onClick={() => navigate(`/cashier/bills/${b.id}`)}>Back to bill</Button></div></Card>
          ) : (
            <>
              <Card>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                  {METHODS.map(({ m: mm, icon: Icon, needsRef }) => { const disabled = mm === 'COMPLIMENTARY' && !canComp; return (
                    <button key={mm} type="button" disabled={disabled} aria-pressed={method === mm} onClick={() => { setMethod(mm); if (!needsRef) setReference(''); }} title={disabled ? 'Requires manager authorization' : undefined}
                      className={cn('min-h-[72px] rounded-md border-2 flex flex-col items-center justify-center gap-1 font-semibold transition-colors disabled:opacity-40', method === mm ? 'border-primary-700 bg-primary-50 text-primary-800' : 'border-neutral-200 bg-white hover:border-neutral-300')}>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {ALT_METHODS.map(({ m: mm, icon: Icon }) => { const av = altAvailable[mm]; return (
                    <button key={mm} type="button" disabled={!av.ok} aria-pressed={method === mm} onClick={() => setMethod(mm)} title={av.ok ? undefined : av.why}
                      className={cn('min-h-[64px] rounded-md border-2 flex flex-col items-center justify-center gap-1 text-sm font-semibold transition-colors disabled:opacity-40', method === mm ? 'border-primary-700 bg-primary-50 text-primary-800' : 'border-neutral-200 bg-white hover:border-neutral-300')}>
              ...
              <Card>
                <div className="flex items-end justify-between gap-3 mb-3">
                  <div><p className="text-label text-neutral-500 uppercase">Amount</p><p className="text-display tabular-nums leading-tight">{money(amt, { decimals: true })}</p></div>
                  <div className="text-right"><p className="text-caption text-neutral-500">Balance due</p><p className="text-xl font-bold tabular-nums text-danger-700">{money(balance, { decimals: true })}</p></div>
                </div>
                <div className="flex flex-wrap gap-2 mb-3">
                  <Button size="sm" variant={amount === '' ? 'primary' : 'outline'} onClick={() => setAmount('')}>Full balance</Button>
                  {[0.5, 0.25].map((f) => <Button key={f} size="sm" variant="outline" onClick={() => setAmount(String(round2(balance * f)))}>{f * 100}%</Button>)}
                  {[500, 1000, 2000].filter((n) => n < balance).map((n) => <Button key={n} size="sm" variant="outline" onClick={() => setAmount(String(n))}>₹{n}</Button>)}
                </div>
                <div className="grid grid-cols-3 gap-2 max-w-sm">
                  {['1', '2', '3', '4', '5', '6', '7', '8', '9', '.', '0', '⌫'].map((k) => <button key={k} type="button" onClick={() => key(k)} className="min-h-pos rounded-sm border border-neutral-200 bg-white text-lg font-semibold hover:bg-neutral-50 active:bg-neutral-100 flex items-center justify-center">{k === '⌫' ? <Delete className="h-5 w-5" /> : k}</button>)}
                </div>
                ...
                {method === 'CASH' && <div className="mt-4 max-w-sm grid grid-cols-2 gap-3 items-end"><Input label="Cash tendered (optional)" type="number" inputMode="decimal" value={tendered} onChange={(e) => setTendered(e.target.value)} />{tendered && <p className="text-sm pb-2">Change: <span className={cn('font-bold tabular-nums', change < 0 ? 'text-danger-700' : 'text-success-700')}>{money(Math.max(change, 0))}</span></p>}</div>}
        ...
        <div className="space-y-4 lg:sticky lg:top-20 self-start">
          <Card><BillSummary bill={b} compact /></Card>
          <Card padded={false}>
            ...
            {b.payments.filter((p) => p.status === 'SUCCESS').length === 0 ? <p className="px-4 py-4 text-sm text-neutral-500">No payments yet.</p> : (
              <ul className="divide-y divide-neutral-100">{b.payments.filter((p) => p.status === 'SUCCESS').map((p) => <li key={p.id} className="px-4 py-2.5 flex items-center justify-between text-sm"><span className="flex items-center gap-2"><Badge size="sm">{PAYMENT_METHOD_LABELS[p.method]}</Badge>{p.reference}</span><span className="font-semibold tabular-nums">{money(p.amount)}</span></li>)}</ul>
            )}
          </Card>
        </div>
      </div>
